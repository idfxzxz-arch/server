# Panduan Instalasi idkcenter

Selamat datang di panduan instalasi **idkcenter**. Panduan ini akan membantu Anda memasang dan menjalankan dashboard pada server atau mesin target Anda (STB, Mini PC, VPS, atau Local Server) yang berbasis sistem operasi Linux.
...
- Membangun dan menjalankan *container* idkcenter secara otomatis.
...
Artinya port 80 sedang dipakai oleh aplikasi lain (seperti Apache atau Nginx bawaan). Matikan aplikasi tersebut dengan `sudo systemctl stop apache2` atau `sudo systemctl stop nginx`, atau ganti port idkcenter (lihat poin 2).
...
**13. Bagaimana cara memperbarui (Update) idkcenter ke versi terbaru?**
